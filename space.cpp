...
#pragma oss declare reduction(mergeCells: CellCounter: mergeCells(omp_in, omp_out)) initializer(omp_priv = CellCounter())
#pragma oss declare reduction(vectorMax: std::vector<float>: vectorMax(omp_in, omp_out)) initializer(omp_priv = omp_orig)
#pragma oss declare reduction(vectorMin: std::vector<float>: vectorMin(omp_in, omp_out)) initializer(omp_priv = omp_orig)
...
void Space::computeCells(float epsilon)
{
    ...
    #pragma oss task for reduction(mergeCells: cellCounter) shared(cellCounter)
...
void Space::computeDimensions(float epsilon)
{
    ...
    #pragma oss task for reduction(vectorMin: minimum) reduction(vectorMax: maximum) shared(minimum) shared(maximum)
